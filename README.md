...
- **复制诊断报告**：一键生成文本报告并复制到剪贴板。
- **必要性分级**：每个运行库标注「必要 / 可选 / 不推荐」，帮助判断该装哪些。
- **全程日志**：所有操作写入 `%LOCALAPPDATA%\VCRedistManager\logs\`。
...
1. 到本仓库 **Releases** 页面下载 `VCRedistManager.exe`（单个文件）。
...
构建产物为 `app\VCRedistManager.exe`。

### 方式三：源码直接运行（开发调试）

```powershell
powershell.exe -NoProfile -STA -ExecutionPolicy Bypass -File .\app\VCRedistManager.ps1
```
...
```
├── app/                         # 程序本体
│   ├── VCRedistManager.exe      # 交付物（单文件 exe，由 Build-Exe.ps1 生成，不入库）
│   ├── VCRedistManager.ps1      # 主程序（WPF 入口，模块无关）
│   ├── VCRedistManager.Core.psm1# 共享基础设施（配置/下载/校验/提权/模块分发）
│   ├── Modules/                 # 运行库模块（自动发现，加一个模块=加一个文件）
│   │   ├── Vc.psm1              #   VC 运行库模块
│   │   └── DirectX.psm1         #   DirectX 运行库模块
│   ├── config.json              # 顶层共享配置 + 各模块配置（下载源/哈希/必要性规则）
│   ├── Bootstrapper.cs          # C# 启动器源码（编译 exe 用）
│   ├── Build-Exe.ps1            # 重新构建 exe 的脚本
│   ├── README.md                # 详细文档（功能/检测/卸载机制/扩展指南/FAQ）
│   └── CLAUDE.md                # 项目约定（模块契约/安全/编码约束）
├── git指南.md                   # 从零推到 GitHub 的分步指南
└── README.md                    # 本文件（仓库首页）
```
...
（待定 —— 建议补充 `LICENSE` 文件，例如 [MIT](https://choosealicense.com/licenses/mit/)。未指定许可证时，他人默认无权限复用本代码。）
